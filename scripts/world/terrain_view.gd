...
func _draw() -> void: # Draws a flat profile-view terrain layer across the entire side-scrolling world.
    draw_rect(Rect2(0.0, SideViewLayout.GROUND_Y, SideViewLayout.WORLD_WIDTH, 110.0), PixelPalette.GROUND, true)
    draw_rect(Rect2(0.0, SideViewLayout.GROUND_Y - 1.0, SideViewLayout.WORLD_WIDTH, 1.0), PixelPalette.GROUND_LIGHT, true)
    for x: int in range(0, int(SideViewLayout.WORLD_WIDTH), 19):
        var y: float = SideViewLayout.GROUND_Y + 14.0 + float((x * 7) % 71)
        draw_rect(Rect2(float(x), y, 2.0, 1.0), PixelPalette.GROUND_LIGHT, true)
    _draw_tree(Vector2(30.0, SideViewLayout.GROUND_Y - 13.0))
    _draw_tree(Vector2(54.0, SideViewLayout.GROUND_Y - 13.0))
    _draw_tree(Vector2(104.0, SideViewLayout.GROUND_Y - 13.0))
    _draw_iron_patch(Vector2(210.0, SideViewLayout.GROUND_Y - 12.0))
    _draw_nickel_patch(Vector2(310.0, SideViewLayout.GROUND_Y - 8.0))
    _draw_cobalt_patch(Vector2(410.0, SideViewLayout.GROUND_Y - 7.0))
