...
func _draw_frame(progress: float) -> void: # Reveals vertical and horizontal structural beams relative to the shared machine origin.
    var height: float = floor(92.0 * progress)
    draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 12.0, SideViewLayout.GROUND_Y - 10.0 - height, 8.0, height), PixelPalette.IRON_LIGHT, true)
    draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 92.0, SideViewLayout.GROUND_Y - 10.0 - height, 8.0, height), PixelPalette.IRON_LIGHT, true)
    if progress > 0.35:
        var top_width: float = floor(80.0 * clampf((progress - 0.35) / 0.65, 0.0, 1.0))
        draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 20.0, SideViewLayout.GROUND_Y - 102.0, top_width, 7.0), PixelPalette.IRON, true)
...
func _draw_drive(progress: float) -> void: # Reveals a large profile-view drive assembly using blocky gears and shafts.
    ...
    if radius <= 0.0:
        return
    var center: Vector2 = Vector2(SideViewLayout.MACHINE_LEFT_X + 56.0, SideViewLayout.GROUND_Y - 41.0)
    draw_circle(center, radius, PixelPalette.INK)
    draw_circle(center, maxf(radius - 5.0, 1.0), PixelPalette.NICKEL)
    draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 53.0, SideViewLayout.GROUND_Y - 80.0, 6.0, 78.0 * progress), PixelPalette.IRON_LIGHT, true)
...
func _draw_controls(progress: float) -> void: # Reveals electronic panels across the upper side of the machine.
    ...
    draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 26.0, SideViewLayout.GROUND_Y - 82.0, 60.0, panel_height), PixelPalette.INK, true)
    if progress > 0.2:
        draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 33.0, SideViewLayout.GROUND_Y - 72.0, 3.0, 3.0), PixelPalette.ELECTRIC, true)
    if progress > 0.45:
        draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 44.0, SideViewLayout.GROUND_Y - 66.0, 2.0, 2.0), PixelPalette.COBALT, true)
    if progress > 0.7:
        draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 75.0, SideViewLayout.GROUND_Y - 74.0, 4.0, 2.0), PixelPalette.ELECTRIC, true)
...
func _draw_active_pixels() -> void: # Adds restrained profile-view indicator pixels once the prototype machine is fully assembled.
    draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 34.0, SideViewLayout.GROUND_Y - 76.0, 3.0, 3.0), PixelPalette.ELECTRIC, true)
    draw_rect(Rect2(SideViewLayout.MACHINE_LEFT_X + 75.0, SideViewLayout.GROUND_Y - 69.0, 2.0, 2.0), PixelPalette.COBALT, true)
